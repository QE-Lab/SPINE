...
static spine::systems::system_2_spin_2_singlet_triplet spin_system = spine::systems::system_2_spin_2_singlet_triplet();
...
bool inHamiltonian(realnum * H, realnum * dt)
{
	if (nsim < Nsim)
	{
		...
		if (nsim < Ngauss) {
			...
			spin_system.setDetuningControl(1, ydetun[nsim - 2 * Ngauss]);
		}
...
void outOperation(complex * U)
{
	...
	probability[nsim] = abs2(U[12*4]) + abs2(U[12*5]) + abs2(U[12*6]) + abs2(U[12*7]);
#ifdef PLOT
